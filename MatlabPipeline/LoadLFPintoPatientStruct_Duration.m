  ...
  for ph=3
    ...
    for rg = 1:length(patient(p).ipsi_region)
      ...
      for lnum = 1:length(patient(p).ipsi_region(rg).lfpnum)
        ...
        if strcmp(patient(p).name, "amyg030") == 1
            tmplfpfn = sprintf('day%dbilateral_lfp_%03d.mat',d,patient(p).ipsi_region(rg).lfpnum(lnum));
        else
            tmplfpfn = sprintf('day%d_lfp_%03d.mat',d,patient(p).ipsi_region(rg).lfpnum(lnum));%load first lfp just to find out how long it is
        end
