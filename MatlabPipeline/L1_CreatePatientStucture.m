...
patient(pID).removeChannels{1} = {'RIFTFd8', 'RM1Cd7', 'RM1Cd8', 'RS1Id7', 'RM1ISd10', 'LMTGAd', 'RS1Id8', 'RS1Id1', 'RM1Id10'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'RIFTFd8', 'RM1Cd7', 'RM1Cd8', 'RS1Id7', 'RM1ISd10', 'LMTGAd', 'RS1Id8', 'RS1Id1', 'RM1Id10'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'RIFTFd8', 'RM1Cd8','RS1Cd1', 'RS1Id8', 'RS1Id1', 'RM1Id10', 'RM1ISd10','RFPd5','LMTGAd'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'2LpRd8', 'LSMACd2', 'LSMACd1', 'LSPFs6', 'LPTs5', '5RpRd8', 'RSTGId7', 'RpHGd7', '6RpRd10', 'Lamyd3', 'LAmyd2'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'2LpRd8', 'LSMACd2', 'LSMACd1', 'LSPFs6', 'LPTs5', '5RpRd8', 'RSTGId7', 'RpHGd7', '6RpRd10', 'Lamyd3', 'LAmyd2'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'LSMACd1', 'LSPFs6', 'LPTs5', '2LpRd8', '2RpRd4', '2RpRd2', '2RpRd5', 'RSTGId7', 'RpHGd7', '5RpRd8', '5RpRd7'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(1).lfpnum = [];%32=CA2/DG per Jon; 31=CA3/DG per Jon (noisy); 30=SUB per Jon (though it is noisy)
patient(pID).ipsi_region(2).lfpnum = [];
patient(pID).ipsi_region(3).lfpnum = 34;%33 = parahippocampal (peri/para border) white matter near collateral eminence per Jon; 34 = PH cortex white matter near collateral sulcus per Jon
patient(pID).ipsi_region(4).lfpnum = [20,21];%stimulated channels = 20 & 21
...
patient(pID).removeChannels{1} = {'L1d9', 'L13d7', 'L15d6', 'L1d7', 'L3d4', 'L13d2', 'L13d1', 'L15d9', 'L19d1', 'LSFs3', 'RSRCH', 'R2d5', 'R2d1', 'LSFs1', 'LATs7', 'R4d1', 'R14d1', 'R12d7', 'R14d8', 'R14d4', 'R20d1', 'R18d7', 'RPTs5', 'R20d10', 'R14d10', 'R16d4'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'L1d9', 'L13d7', 'L15d6', 'L1d7', 'L3d4', 'L13d2', 'L13d1', 'L15d9', 'L19d1', 'LSFs3', 'RSRCH', 'R2d5', 'R2d1', 'LSFs1', 'LATs7', 'R4d1', 'R14d1', 'R12d7', 'R14d8', 'R14d4', 'R20d1', 'R18d7', 'RPTs5', 'R20d10', 'R14d10', 'R16d4'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'L15d2', 'R10d3', 'R12d7', 'R14d1', 'R14d8', 'R16d4', 'R20d10', 'R4d1', 'R2d1', 'R2d5', 'LATs7', 'LATs5', 'RPTs5', 'R18d7', 'R14d10', 'R14d4', 'L13d7', 'L1d9', 'L1d7', 'L15d9', 'L15d6', 'L3d4', 'LSFs3', 'LSFs1', 'L19d1'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(3).lfpnum = 108;%border of perirhinal ctx and adjacent white matter per Jon; 108 is same per Jon
...
patient(pID).removeChannels{1} = {'R2Od7', 'C127', 'C128', 'C126', 'O1', 'T5', 'T3', 'F7', 'Fp1', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'R2Od9', 'LAmyd1', 'LAmyd3', 'RAHd1', 'R2FOpd7', 'RPTrid7'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'R2Od7', 'C127', 'C128', 'C126', 'O1', 'T5', 'T3', 'F7', 'Fp1', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'R2Od9', 'LAmyd1', 'LAmyd3', 'RAHd1', 'R2FOpd7', 'RPTrid7'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'C128', 'C127', 'C126', 'O1', 'T5', 'T3', 'F7', 'Fp1', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'R2Od7', 'RAHd1', 'R2FOpd7', 'RPTrid7'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'LAmyd1', 'LAmyd3', 'LAmyd7', 'LAmyd2', 'LAHd1', 'RPHd2', 'RPHd4', 'RSMAd7'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'LAmyd1', 'LAmyd3', 'LAmyd7', 'LAmyd2', 'LAHd1', 'RPHd2', 'RPHd4', 'RSMAd7'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'LAmyd7', 'LAmyd1', 'RPHd4', 'RACind1', 'RSMAd7', 'RACind3', 'RAHd2', 'RAmyd3', 'LpHGd3', 'LAHd2'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(2).lfpnum = [];
patient(pID).ipsi_region(3).lfpnum = 52;%Per Jon: 49, 50, 51 = entorhinal cortex; 52, 55, 56, 57 = parahip/entorhinal, 52 is close to PHC/PRC while 55,56,57 are in perirhinal
patient(pID).ipsi_region(4).lfpnum = [53,54];%stimulated channels = 53 & 54--53 was noisy - Martina: 53 and 54 are both noisy at day2
patient(pID).ipsi_region(5).lfpnum = [27,24,25,28,31];%region 5 = all possible hippocampus electrodes; see above for notes
patient(pID).ipsi_region(6).lfpnum = [49,50,51]; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [55,56,57];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'RAHcd3', 'RPHcd8', 'RMTGd8', 'RROI6d9', 'RROI6d7', 'RROI6d5', 'RROI6d3', 'RROI6d1', 'RROI4d10', 'RROI4d3', 'RPHcd5', 'REntd5', 'REntd6', 'LPHcd1', 'LPHcd4', 'LPHcd2', 'RAHcd2', 'RPHcd4', 'RPHcd3', 'RPHcd2', 'RAHcd5', 'RAHcd4', 'RAHcd1'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'RAHcd3', 'RPHcd8', 'RMTGd8', 'RROI6d9', 'RROI6d7', 'RROI6d5', 'RROI6d3', 'RROI6d1', 'RROI4d10', 'RROI4d3', 'RPHcd5', 'REntd5', 'REntd6', 'LPHcd1', 'LPHcd4', 'LPHcd2', 'RAHcd2', 'RPHcd4', 'RPHcd3', 'RPHcd2', 'RAHcd5', 'RAHcd4', 'RAHcd1'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'RAHcd3', 'REntd6', 'RROI4d3', 'RROI6d9', 'RROI6d7', 'RROI6d5', 'RROI6d3', 'RROI6d1', 'LPHcd1', 'LPHcd2', 'RPHcd4', 'RPHcd5', 'RPHcd8', 'RMTGd8', 'RROI4d10'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).samprate = 999.41211;
patient(pID).ipsi_region(1).lfpnum = 34;%per Jon: 21=uncinate gyrus/head; 31,32,42=CA3/fimbria;33=CA2/CA3; 34 = CA2/CA1 per Joe via ROSA: 21=head; 31=anterior in hip*; 32=fimbria; 33=mid S-T; 34 = mid S-T; 42 = post 1/3 S-T per DARPA:22-25 are CA1/CA3; 34 and 43 are also CA1/CA3 per DARPA
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'R4MHd10', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'C113', 'C112', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'C103', 'C104', 'C102', 'C100', 'C101', 'C99', 'C98', 'C97', 'C96', 'C93', 'C94', 'C95', 'C92', 'C91', 'C90', 'C89', 'C87', 'C88', 'C86', 'C85', 'C84', 'C83', 'C82', 'C80', 'C81', 'C79', 'C78', 'C77', 'C76', 'C74', 'C75', 'C73', 'C72', 'C71', 'L3Amyd3', 'L3Amyd2', 'L5AHd1', 'L5AHd2', 'L7MHd3', 'L7MHd2', 'L7MHd1', 'L5AHd4', 'L5AHd3', 'L1TPol1', 'L1TPol2', 'L9PHd1', 'L9PHd2', 'L7MHd4'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'R4MHd10', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'C113', 'C112', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'C103', 'C104', 'C102', 'C100', 'C101', 'C99', 'C98', 'C97', 'C96', 'C93', 'C94', 'C95', 'C92', 'C91', 'C90', 'C89', 'C87', 'C88', 'C86', 'C85', 'C84', 'C83', 'C82', 'C80', 'C81', 'C79', 'C78', 'C77', 'C76', 'C74', 'C75', 'C73', 'C72', 'C71', 'L3Amyd3', 'L3Amyd2', 'L5AHd1', 'L5AHd2', 'L7MHd3', 'L7MHd2', 'L7MHd1', 'L5AHd4', 'L5AHd3', 'L1TPol1', 'L1TPol2', 'L9PHd1', 'L9PHd2', 'L7MHd4'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'C118', 'C117', 'C116', 'C115', 'C114', 'C113', 'C112', 'C111', 'C110', 'C108', 'C109', 'C107', 'C106', 'C105', 'C104', 'C103', 'C102', 'C101', 'C100', 'C98', 'C99', 'C97', 'C96', 'C95', 'C94', 'C93', 'C92', 'C91', 'C90', 'C89', 'C88', 'C87', 'C86', 'C85', 'C84', 'C83', 'C82', 'C81', 'C79', 'C80', 'C78', 'C77', 'C76', 'C75', 'C74', 'C73', 'C72', 'C71', 'R4MHd10', 'L5AHd1', 'L5AHd3', 'L5AHd2', 'L5AHd4', 'L7MHd3', 'L7MHd2', 'L7MHd1', 'L9PHd2', 'L9PHd1', 'L1TPol2', 'L1TPol1', 'L1TPol4', 'L1TPol3'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(3).lfpnum = 35;%per Jon: 35 = Parahip gyrus/fusiform gyrus/perirhinal per DARPA: 35 and 36 are perirhinal
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'L5Amyd3', 'L13OF1d9', 'L13OF1d8', 'C128', 'C127', 'C126', 'C124', 'C125', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'C113', 'C112', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'C104', 'C103', 'C102', 'C101', 'C100', 'C99', 'C98', 'C97', 'C96', 'C95', 'C94', 'C93', 'C92', 'C91', 'C89', 'C90', 'C88', 'C87', 'C86', 'C85', 'C84', 'C83', 'C82', 'C81', 'L5Amyd2'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'L5Amyd3', 'L13OF1d9', 'L13OF1d8', 'C128', 'C127', 'C126', 'C124', 'C125', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C114', 'C113', 'C112', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'C104', 'C103', 'C102', 'C101', 'C100', 'C99', 'C98', 'C97', 'C96', 'C95', 'C94', 'C93', 'C92', 'C91', 'C89', 'C90', 'C88', 'C87', 'C86', 'C85', 'C84', 'C83', 'C82', 'C81', 'L5Amyd2'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'C127', 'C128', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C116', 'C115', 'C113', 'C114', 'C112', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'C103', 'C104', 'C102', 'C101', 'C100', 'C97', 'C99', 'C98', 'C96', 'C95', 'C94', 'C93', 'C92', 'C91', 'C90', 'C89', 'C87', 'C88', 'C81', 'C83', 'C84', 'C85', 'C86', 'C82', 'L13OF1d8', 'L11PHd8'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(4).lfpnum = [29,30];%stimulated channels = 29 & 30
patient(pID).ipsi_region(5).lfpnum = [35,34,40];%region 5 = all possible hippocampus electrodes; see above for notes
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'LHcd9', 'RAmyd1', 'RPHcd1', 'C127', 'RSupPs1', 'RSupPs3', 'RSupPs5', 'RSupPs7', 'RSupPs9', 'C128', 'RAntTs9', 'RPosTs3', 'RAntTs3', 'RAntTs5', 'RPHcd9', 'RAmyd3', 'RPHcd5', 'RPHcd7'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'LHcd9', 'RAmyd1', 'RPHcd1', 'C127', 'RSupPs1', 'RSupPs3', 'RSupPs5', 'RSupPs7', 'RSupPs9', 'C128', 'RAntTs9', 'RPosTs3', 'RAntTs3', 'RAntTs5', 'RPHcd9', 'RAmyd3', 'RPHcd5', 'RPHcd7'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'LHcd9', 'RPHcd1', 'RPHcd9', 'RPHcd3', 'RAHcd3', 'RAmyd9', 'RAmyd7', 'RSupPs9', 'RSupPs7', 'RSupPs5', 'RSupPs3', 'RSupPs1', 'RPosTs3'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
%Martina: CA analysis channel (24) is very epileptic, Martina removed it
pID = pID+1;
patient(pID).exp = 'Original';
...
patient(pID).ipsi_region(2).lfpnum = 13;%per Jon: 11 =uncal apex DG, 13, 23 = DG; per DARPA: 13 = DG
...
patient(pID).ipsi_region(4).lfpnum = [2,3];%stimulated channels = 2 & 3, -->once called #1, but corrected on 8/5/16
patient(pID).ipsi_region(5).lfpnum = [13,25];%region 5 = all possible hippocampus electrodes; see above for notes -Martina: all other channels are very epileptic, including the 24, the CA1 channel used for Joe's analysis
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels - 22 EC but super epileptic
patient(pID).ipsi_region(7).lfpnum = 34;%perirhinal cortex channels
patient(pID).removeChannels{1} = {'LAmyd2', 'C126', 'C128', 'C127', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C114', 'C115', 'C116', 'C112', 'C113', 'C111', 'C110', 'C109', 'C108', 'C105', 'C104', 'C103', 'C102', 'C101', 'C107', 'C106', 'LAHd2', 'LAHd1', 'LAHd3', 'LMHd4', 'LPHd2', 'LAmyd3'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'LAmyd2', 'C126', 'C128', 'C127', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C114', 'C115', 'C116', 'C112', 'C113', 'C111', 'C110', 'C109', 'C108', 'C105', 'C104', 'C103', 'C102', 'C101', 'C107', 'C106', 'LAHd2', 'LAHd1', 'LAHd3', 'LMHd4', 'LPHd2', 'LAmyd3'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'LAHd3', 'LAHd2', 'C128', 'C127', 'C125', 'C126', 'C124', 'C121', 'C123', 'C122', 'C120', 'C119', 'C118', 'C117', 'C116', 'C114', 'C115', 'C113', 'C112', 'C111', 'C110', 'C107', 'C109', 'C108', 'C106', 'C105', 'C104', 'C103', 'RSRCH2', 'RSRCH1', 'RMHd3'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).samprate =  499.70714;
patient(pID).ipsi_region(1).lfpnum = [10,32];%per Jon: 10=uncinate gyrus/head; 20=CA3/fimbria; 21=CA2; 32=CA1/2; 30,31=sub, per Joe via ROSA: 10 = head near both white matter and ventricle; 20=mid ST; 21*=mid ST; 30,31,32 = posterior 1/3 S-T...per DARPA: 10, 11, 12 = CA1/CA2; 21, 31, 32, 33 = CA1/CA3
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'LOFMed7', 'LOFLd9', 'LOFMid1', 'LMHd4', 'LAmyd2', 'LAmyd1', 'LMHd2', 'LAHd2', 'LAHd1', 'LMHd1', 'LPHd3', 'LPHd2'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'LOFMed7', 'LOFLd9', 'LOFMid1', 'LMHd4', 'LAmyd2', 'LAmyd1', 'LMHd2', 'LAHd2', 'LAHd1', 'LMHd1', 'LPHd3', 'LPHd2'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'LOFLd9', 'LMHd1', 'LMHd2', 'LAHd1', 'LAHd2', 'LAmyd1', 'LPHd3', 'LPHd2', 'RAmyd1'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).removeChannels{1} = {'LAmyd10', 'LATs1', 'LAmyd9', 'LAmyd2', 'LAmyd3', 'LAHd3', 'LAHd2', 'LAmyd1', 'LAHd1', 'LAHd4', 'LMHd3', 'LMHd2', 'LMHd1', 'RAHd9', 'LMHd7'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'LAmyd10', 'LATs1', 'LAmyd9', 'LAmyd2', 'LAmyd3', 'LAHd3', 'LAHd2', 'LAmyd1', 'LAHd1', 'LAHd4', 'LMHd3', 'LMHd2', 'LMHd1', 'RAHd9', 'LMHd7'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).ipsi_region(6).lfpnum = []; %entorhinal cortex channels
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} = {'L1d1', 'L1d5', 'L3d1', 'L5d9', 'L7d3', 'L7d7', 'L17d1', 'L13d5', 'R4d1', 'L17d7', 'L19d5', 'LSPs7', 'RFPs5', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'R4d3', 'L25d9'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'L1d1', 'L1d5', 'L3d1', 'L5d9', 'L7d3', 'L7d7', 'L17d1', 'L13d5', 'R4d1', 'L17d7', 'L19d5', 'LSPs7', 'RFPs5', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'R4d3', 'L25d9'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'L11d9', 'L17d1', 'L13d5', 'LSPs7', 'LSPs5', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'L25d9'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).samprate =  499.70714;
patient(pID).ipsi_region(1).lfpnum = 1;%per Jon: 1=ambient gyrus of uncus; 21=ventricle near head of hip; 22=ventricle above CA2...per DARPA: 11, 12, 13, 21, 22, 23 = CA1; 11 is noisy
...
patient(pID).removeChannels{1} = {'L1d3', 'L3d1', 'L3d7', 'L3d8', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C116', 'C117', 'C113', 'C115', 'C114', 'C112', 'C111', 'C105', 'C106', 'C107', 'C108', 'C109', 'C110', 'R4d8', 'R10d5', 'RFs6', 'R4d1', 'R10d10', 'L1d10', 'L1d2', 'R2d9'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'L1d3', 'L3d1', 'L3d7', 'L3d8', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C116', 'C117', 'C113', 'C115', 'C114', 'C112', 'C111', 'C105', 'C106', 'C107', 'C108', 'C109', 'C110', 'R4d8', 'R10d5', 'RFs6', 'R4d1', 'R10d10', 'L1d10', 'L1d2', 'R2d9'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'L3d7', 'L3d8', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C115', 'C117', 'C116', 'C112', 'C114', 'C113', 'C111', 'C110', 'C109', 'C108', 'C107', 'C106', 'C105', 'R10d5', 'R6d1', 'R10d10', 'R4d1', 'R2d9', 'RFs6', 'R4d8', 'R10d3', 'R10d4', 'L3d1', 'L1d10', 'R2d1', 'R2d2'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];
patient(pID).contra_region(2).lfpnum = [];
patient(pID).contra_region(3).lfpnum = [];
patient(pID).contra_region(4).lfpnum = [];
...
patient(pID).removeChannels{1} = {'8Rd7', '26Rd10', '30Rd1', '20Rd10', '20Rd9', '20Rd8', '28Rd8', '22Rd10', 'DC16', 'DC15', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC8', 'DC2', 'DC1', 'DC3', 'DC4', 'DC5', 'DC7', 'DC6', 'OSAT', 'PR', 'Pleth', 'TRIG', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', '32Rd1', '32Rd4', '18Rd8', '2Rd7', '2Rd4'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'8Rd7', '26Rd10', '30Rd1', '20Rd10', '20Rd9', '20Rd8', '28Rd8', '22Rd10', 'DC16', 'DC15', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC8', 'DC2', 'DC1', 'DC3', 'DC4', 'DC5', 'DC7', 'DC6', 'OSAT', 'PR', 'Pleth', 'TRIG', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', '32Rd1', '32Rd4', '18Rd8', '2Rd7', '2Rd4'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'8Rd7', '32Rd1', 'DC10', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'C128', 'C127', 'C126', 'C125', 'C124', 'C123', 'C122', '2Rd7', '2Rd4', '18Rd8', '26Rd10', '22Rd10', '28Rd8', '30Rd5', '20Rd10', '20Rd9', '20Rd8', '30Rd6'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
...
patient(pID).removeChannels{1} = {'1Ld1', '1Ld7', '5Ld1', '5Ld4', '11Ld1', '13Ld1', '15Ld8', '15Ld7', '15Ld9', '17Ld5', '17Ld6', '1Ld2', '5Ld2', '3Ld3', '3Ld2', '3Ld1', '5Ld3', '3Ld4', 'DC15', 'DC13', 'DC16', 'TRIG', 'DC14', 'DC11', 'DC10', 'DC12', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'Pleth', 'PR', 'OSAT', '21Ld5', '25Ld6', '6Rd1', 'EKG2', 'EKG1', '21Ld7', '2Rd1', '2Rd3', '4Rd3', '4Rd1', '6Rd3', '19Ld5', '23Ld1', '4Rd5'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {'1Ld1', '1Ld7', '5Ld1', '5Ld4', '11Ld1', '13Ld1', '15Ld8', '15Ld7', '15Ld9', '17Ld5', '17Ld6', '1Ld2', '5Ld2', '3Ld3', '3Ld2', '3Ld1', '5Ld3', '3Ld4', 'DC15', 'DC13', 'DC16', 'TRIG', 'DC14', 'DC11', 'DC10', 'DC12', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'Pleth', 'PR', 'OSAT', '21Ld5', '25Ld6', '6Rd1', 'EKG2', 'EKG1', '21Ld7', '2Rd1', '2Rd3', '4Rd3', '4Rd1', '6Rd3', '19Ld5', '23Ld1', '4Rd5'}; % Channels that has to be removed before computing the median for phase = 2 (Day1_ImmTest)
patient(pID).removeChannels{3} = {'1Ld7', '5Ld4', '11Ld1', '3Ld2', '3Ld3', '3Ld1', '5Ld2', '5Ld1', '15Ld8', '15Ld7', '13Ld2', '13Ld1', '15Ld9', '1Ld3', '17Ld5', '17Ld9', '13Ld4', '5Ld5', '5Ld3', 'DC15', 'TRIG', 'DC16', 'DC14', 'DC13', 'DC11', 'DC9', 'DC8', 'DC12', 'DC10', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'EKG2', 'EKG1', 'Pleth', 'PR', 'OSAT', '6Rd1', '21Ld5', '25Ld6', '19Ld5', '6Rd3', '2Rd3', '2Rd1', '4Rd3', '4Rd5', '4Rd1', '6Rd5'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
...
% patient(pID).removeChannels{1} = {}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
...
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'5Ld BF10', '5Ld BF9', '5Ld BF8', '5Ld BF5', '5Ld BF6', '5Ld BF3', '5Ld BF15', '7Ld6', '15Ld7', '19Ld7', '21Ld1', '21Ld4', 'Pleth', 'PR', 'OSAT', 'TRIG', 'DC16', 'DC15', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'C128', 'DC2', 'DC1', '23Ld10', '23Ld9', '23Ld8', 'EKG', '23Ld7', '23Ld5', '5Ld BF2', '9Ld4', '3Ld6', '5Ld BF4', '5Ld BF7', '11Ld6', '1Ld1', '1Ld2', '1Ld9', '3Ld1', '3Ld5', '1Ld10', '1Ld3', '1Ld8'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
...
patient(pID).ipsi_region(7).lfpnum = [];%perirhinal cortex channels
patient(pID).removeChannels{1} ={'2Rd5', 'Pleth', 'PR', 'OSAT', 'DC16', 'DC15', 'TRIG', 'DC14', 'DC13', 'DC11', 'DC10', 'DC12', 'DC7', 'DC6', 'DC5', 'DC9', 'DC8', 'DC4', 'DC3', 'DC2', 'DC1', 'C128', 'C127', 'C116', 'C126', 'C125', 'C115', 'C124', 'C123', 'C121', 'C114', 'C119', 'C118', 'C120', 'C113', 'C122', 'C112', 'C111', 'C117', 'C110', 'C109', 'P4', 'C3', 'F3', 'O1', 'T5', 'T3', 'F7', 'Fp1', 'EKG', '2Rd1', '2Rd2'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'PR', 'DC16', 'C114', 'C112', 'C113', 'Pleth', 'OSAT', 'TRIG', 'DC15', 'C111', 'C110', 'C109', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC6', 'DC8', 'DC7', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'C127', 'C126', 'C128', 'C125', 'C124', 'C123', 'C122', 'C121', 'C120', 'C119', 'C118', 'C117', 'C115', 'C116', 'P4', 'C3', 'F3', 'O1', 'T5', 'T3', 'F7', 'Fp1', 'EKG', '2Rd5', '2Rd2', '2Rd1'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];%hippocampal CA fields
patient(pID).contra_region(2).lfpnum = [];%DG
patient(pID).contra_region(3).lfpnum = [];%PHG
patient(pID).contra_region(4).lfpnum = [];%amygdala stim
...
patient(pID).removeChannels{1} = {'11Ld1', '11Ld2', '1Ld3', '1Ld2', 'DC16', 'DC15', 'DC14', 'DC13', 'OSAT', 'PR', 'TRIG', 'DC12', 'DC11', 'DC10', 'DC9', 'Pleth', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'RSRCH', 'EKG', '23Ld3'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'7Ld1', '11Ld2', '11Ld1', '9Ld1', '7Ld10', 'DC10', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'RSRCH', 'DC2', '23Ld3', 'DC3', 'DC1', 'EKG', '23Ld8'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];%hippocampal CA fields
patient(pID).contra_region(2).lfpnum = [];%DG
patient(pID).contra_region(3).lfpnum = [];%PHG
patient(pID).contra_region(4).lfpnum = [];%amygdala stim
...
patient(pID).removeChannels{1} = {'1Ld2', '9Ld9', '9Ld10', '1Ld3', '3Ld2', '3Ld4', '3Ld3', '3Ld1', '5Ld4', '5Ld3', '5Ld2', '11Ld4', '11Ld2', '11Ld5', '11Ld3', '1Ld1', '21Ld1', '2Rd5', '2Rd4', '2Rd3', '2Rd2', '4Rd2', '4Rd4', '4Rd3', '4Rd1', '6Rd3', '6Rd4', '6Rd2'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'3Ld4', '3Ld3', '3Ld2', '3Ld1', '5Ld3', '5Ld2', '5Ld4', '9Ld10', '9Ld9', '15Ld1', '4Rd3', '4Rd2', '4Rd1', '4Rd5', '2Rd2', '2Rd3', '4Rd4', '6Rd5', '6Rd3', '6Rd2', '6Rd4', '21Ld1', '16Rd2', '16Rd1', 'PR', 'DC15', 'DC14', 'DC13', 'DC16', 'Pleth', 'OSAT', 'TRIG', 'DC12', 'DC11', 'DC10', 'DC8', 'DC7', 'DC9', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'C256', 'C255', 'C254', 'C253', 'C252', 'C251', 'C250', 'C249', 'C248', 'C247', 'C246', 'C245', 'C244', 'C243', 'C242', 'C241', 'C239', 'C238', 'C240', 'C237', 'C236', 'C235', 'C234', 'C232', 'C222', 'C231', 'C233', 'C230', 'C229', 'C228', 'C227', 'C226', 'C225', 'C224', 'C223', 'EKG'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
...
patient(pID).removeChannels{1} = {'28Rd2', 'DC10', 'DC9', 'DC8', 'DC7', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'DC6', 'EKG', 'Rsch', '2Rd3', '2Rd2', '4Rd1', '2Rd1', '6Rd4', '6Rd3', '12Rd3', '12Rd2', '12Rd1'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'14Rd9', '28Rd3', '28Rd7', '2Rd1', '2Rd2', '28Rd2', '28Rd1', '28Rd8', 'PR', 'OSAT', 'Pleth', 'DC16', 'TRIG', 'DC15', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'EKG', 'Rsch', '8Rd10', '4Rd6', '4Rd1', '28Rd6', '28Rd5', '28Rd4'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];%hippocampal CA fields
patient(pID).contra_region(2).lfpnum = [];%DG
patient(pID).contra_region(3).lfpnum = [];%PHG
patient(pID).contra_region(4).lfpnum = [];%amygdala stim
...
patient(pID).removeChannels{1} = {'1Ld3', '3Ld13', '6Rd12', '1Ld4', '3Ld6', '3Ld3', '3Ld2', '3Ld4', '3Ld5', '3Ld1', '1Ld15', '3Ld15', 'DC2', 'DC1', 'EKG', '22Rd14', '28Rd7', '28Rd8', '18Rd15', '20Rd18', '20Rd17'}; % Channels that has to be removed before computing the median for phase = 1 (Day1_Study)
patient(pID).removeChannels{2} = {}; % no imm test for this subject, leave blank
patient(pID).removeChannels{3} = {'1Ld15', '28Rd7', 'Pleth', 'PR', 'OSAT', 'TRIG', 'DC16', 'DC15', 'DC14', 'DC13', 'DC12', 'DC11', 'DC10', 'DC9', 'DC8', 'DC7', 'DC6', 'DC5', 'DC4', 'DC3', 'DC2', 'DC1', 'EKG', '22Rd14', '18Rd15', '20Rd17', '20Rd18', '18Rd3', '18Rd12', '6Rd11', '6Rd12', '3Ld13', '3Ld15', '1Ld14', '2Rd13', '1Ld13', '1Ld12'}; % Channels that has to be removed before computing the median for phase = 3 (Day2)
patient(pID).contra_region(1).lfpnum = [];%hippocampal CA fields
