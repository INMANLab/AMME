%% Move data from ph=2 to ph=3
for pId = 1:length(patient)
    if (~strcmp(patient(pId).exp,"Timing"))
        continue
    end
    patient(pId).phase(3) = patient(pId).phase(2); 
end
...
%         fprintf('LFP file name is %s\n',tmplfpfn)
%       end
%     if(strcmp(tmplfpfn,regexprep(patient(p).syncfn,'day1','day2')))
%         continue;
%     end
  ...
  for ph=3
    ...
    for rg = 1:length(patient(p).ipsi_region)
      ...
      for lnum = 1:length(patient(p).ipsi_region(rg).lfpnum)
        ...
        if strcmp(patient(p).name, 'amyg045') == 1 || strcmp(patient(p).name, 'amyg048') == 1 || strcmp(patient(p).name, 'amyg054') == 1 || strcmp(patient(p).name, 'amyg060') == 1 || strcmp(patient(p).name, 'amyg072') == 1
            tmplfpfn = sprintf('day%dLamyg_lfp_%03d.mat',d,patient(p).ipsi_region(rg).lfpnum(lnum));
        elseif strcmp(patient(p).name, 'amyg057') ==1
            tmplfpfn = sprintf('day%dbipolar_lfp_%03d.mat',d,patient(p).ipsi_region(rg).lfpnum(lnum));
        else
            tmplfpfn = sprintf('day%d_lfp_%03d.mat',d,patient(p).ipsi_region(rg).lfpnum(lnum));
        end
