...
for rg1 = 1:length(eegDat)
    ...
    for  chIdx1 = 1:size(eegDatPost1,3)
        [~,pVal1Post,~,freqsPost] = ComputeSegmentedCoherency(eegDatPost1(:,:,chIdx1), eegDatPost1(:,:,chIdx1), multitaperPar);
        [~,pVal1Pre,~,freqsPre] = ComputeSegmentedCoherency(eegDatPre1(:,:,chIdx1), eegDatPre1(:,:,chIdx1), multitaperPar);
    ...
    for rg2 = (rg1+1):length(eegDat)
        ...
        for  chIdx1 = 1:size(eegDatPost1,3)
            for  chIdx2 = 1:size(eegDatPost2,3)
                [cValPost,~,~,freqsPost] = ComputeSegmentedCoherency(eegDatPost1(:,:,chIdx1), eegDatPost2(:,:,chIdx2), multitaperPar);
                % plot(freqsPre,pVal1Pre(:,1));
                [cValPre,~,~,freqsPre] = ComputeSegmentedCoherency(eegDatPre1(:,:,chIdx1), eegDatPre2(:,:,chIdx2), multitaperPar);
