...
elseif(istable(trialDat))
    emptyTrials = isnan(trialDat.start_time);
    dat = trialDat(~emptyTrials,:);
    trialCount = size(dat,1);
    dat = horzcat(dat.region{:,rgIdx}); %Create a 2d matrix of the eeg data: sample x (channels x trials)
end
