%% Initialize
%function recogtest = plot_test_power_coherence_plots_for_sfn16(patient)
%used tempdata_10_10_16.mat
%plots only five patients with electrodes in hip, BLA, and perirhinal ctx
%JRM 11/4/16
...
for ph=3%phase 2 and 3 = immediate and one-day tests, respectively
  ...
  for p = 3:length(patient)
    ...
    if(length(patient(p).phase(ph).trial_start_times)==length(patient(p).phase(ph).trial) && ~isempty(patient(p).region(rg1touse).lfpnum))
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MIDDLE ROW  = COHERENCE  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%let's plot absolute mean (across participants) coherece for each region pair averaged across trial types
for testnum=1:2
  subplot(3,4,testnum+4), hold on
  
  %HIPPOCAMPUS-PERIRHINAL COHERENCE
  %recogtest(testnum).all_H_PERI_C(pctr,ftouse,tt)
  tmpmean = mean(recogtest(testnum).all_H_PERI_C, 3);%mean across trial types
  tmpsem = std(tmpmean)/sqrt(length(tmpmean(:,1)));%sem across patients
  tmpmean = mean(tmpmean);%mean across patients
  error_fill_plot(recogtest(testnum).f, tmpmean, tmpsem, regioncolor{1});
  
  %HIPPOCAMPUS-BLA COHERENCE
  %recogtest(testnum).all_H_BLA_C(pctr,ftouse,tt)
  tmpmean = mean(recogtest(testnum).all_H_BLA_C, 3);%mean across trial types
  tmpsem = std(tmpmean)/sqrt(length(tmpmean(:,1)));%sem across patients
  tmpmean = mean(tmpmean);%mean across patients
  error_fill_plot(recogtest(testnum).f, tmpmean, tmpsem, regioncolor{2});
  
  %BLA-PERIRHINAL COHERENCE
  %recogtest(testnum).all_BLA_PERI_C(pctr,ftouse,tt)
  tmpmean = mean(recogtest(testnum).all_BLA_PERI_C, 3);%mean across trial types
  tmpsem = std(tmpmean)/sqrt(length(tmpmean(:,1)));%sem across patients
  tmpmean = mean(tmpmean);%mean across patients
  error_fill_plot(recogtest(testnum).f, tmpmean, tmpsem, regioncolor{3});
  ...
  ylim([.2 1])
  set(gca,'XTick',0:20:100)
  ylabel('Coherehce'); xlabel('Frequency (Hz)');
  title('Coherence')
  set(gca, 'FontName', 'Arial', 'FontSize', 20)
  set(gca, 'FontWeight', 'bold')
  set(gca, 'LineWidth', 1)
...
plot([5 15], [70 70], 'Color', rgb(regioncolor{2}), 'LineWidth', 4)
plot([5 15], [50 50], 'Color', rgb(regioncolor{3}), 'LineWidth', 4)
text(20,90, 'Hipp-BLA', 'Color', rgb(regioncolor{1}), 'FontName', 'Arial', 'FontSize', 20)
text(20,70, 'Peri-Hipp', 'Color',  rgb(regioncolor{2}), 'FontName', 'Arial', 'FontSize', 20)
text(20,50, 'BLA-Peri', 'Color', rgb(regioncolor{3}), 'FontName', 'Arial', 'FontSize', 20)
set(gca, 'FontWeight', 'bold')
set(gca,'YTick',[])
set(gca,'XTick',[])
axis off
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% THIRD ROW  = PERI POWER DIFFS  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%let's plot absolute mean (across participants) coherece for each region pair averaged across trial types
for testnum=1:2
  subplot(3,4,testnum+8), hold on
  
  %PERIRHINAL POWER - PRETRIAL BASELINE
  for tt = 3:-1:1
    tmpmean = recogtest(testnum).all_PERI_diff_P(:,:,tt);%this freq range for this trial type
    tmpsem = std(tmpmean)/sqrt(length(tmpmean(:,1)));%sem across patients
    tmpmean = mean(tmpmean);%mean across patients
    error_fill_plot(recogtest(testnum).f, tmpmean, tmpsem, ttcolor{tt});
  end
  ...
  set(gca,'XTick',0:20:100)
  ylabel('Power (dB)'); xlabel('Frequency (Hz)');
  title('Perirhinal Power-Baseline')
  set(gca, 'FontName', 'Arial', 'FontSize', 20)
  set(gca, 'FontWeight', 'bold')
  set(gca, 'LineWidth', 1)
end%testnum
...
ylim([0 100])
xlim([0 100])
plot([5 15], [90 90], 'Color', rgb('Red'), 'LineWidth', 4)
plot([5 15], [70 70], 'Color', rgb('Blue'), 'LineWidth', 4)
plot([5 15], [50 50], 'Color', rgb('Green'), 'LineWidth', 4)
text(20,90, 'Stimulation', 'Color', rgb('Red'), 'FontName', 'Arial', 'FontSize', 20)
text(20,70, 'No Stimulation', 'Color',  rgb('Blue'), 'FontName', 'Arial', 'FontSize', 20)
text(20,50, 'New', 'Color', rgb('Green'), 'FontName', 'Arial', 'FontSize', 20)
set(gca, 'FontWeight', 'bold')
set(gca,'YTick',[])
set(gca,'XTick',[])
axis off
...
%     PLOT MEAN POWER AND COHERENCE ACROSS PARTICIPANTS   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('name', 'MAIN SFN 2016 FIG #2', 'Color', [1 1 1]);
set(gcf,'DefaultLineLineWidth',1)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TOP ROW  = COHERENCE BY CONDITION  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%let's plot absolute mean (across participants) coherece for each region pair averaged across trial types
for testnum=1:2
  subplot(3,4,testnum), hold on
  ...
  %HIPP-PERIRHINAL COHERRENCE - PRETRIAL BASELINE
  for tt = 3:-1:1
    tmpmean = recogtest(testnum).all_H_PERI_diff_C(:,:,tt);%this freq range for trial type
    tmpsem = std(tmpmean)/sqrt(length(tmpmean(:,1)));%sem across patients
    tmpmean = mean(tmpmean);%mean across patients
    error_fill_plot(recogtest(testnum).f, tmpmean, tmpsem, ttcolor{tt});
  end
...
alldata = zeros(5,9,3);%subjects by subplot by anatomy 
...
for testnum=1:2
  ...
  whichftouse = logical(recogtest(testnum).f>=30) & logical(recogtest(testnum).f<=55);
  tmpfreqmean = zeros(1,3);
  tmpfreqsem = zeros(1,3);
...
annotation('TextBox', [.5, .6, .1, .1], 'String', 'Stimulation-No Stimulation',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 30,'FontWeight', 'bold', 'LineStyle', 'none')


annotation('TextBox', [.25, .5, .1, .1], 'String', 'Immediate',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 24,'FontWeight', 'bold', 'LineStyle', 'none')

annotation('TextBox', [.7, .5, .1, .1], 'String', 'One-Day',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 24,'FontWeight', 'bold', 'LineStyle', 'none')


annotation('TextBox', [.15, .45, .1, .1], 'String', 'Power',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 20,'FontWeight', 'bold', 'LineStyle', 'none')

annotation('TextBox', [.35, .45, .1, .1], 'String', 'Coherence',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 20,'FontWeight', 'bold', 'LineStyle', 'none')


annotation('TextBox', [.6, .45, .1, .1], 'String', 'Power',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 20,'FontWeight', 'bold', 'LineStyle', 'none')

annotation('TextBox', [.8, .45, .1, .1], 'String', 'Coherence',...
  'HorizontalAlignment', 'center', 'FontName', 'Arial', 'FontSize', 20,'FontWeight', 'bold', 'LineStyle', 'none')
...
%legend
subplot(2,9,7), hold on
ylim([0 100])
xlim([0 100])
plot([5 15], [90 90], 'Color', rgb(regioncolor{1}), 'LineWidth', 4)
plot([5 15], [70 70], 'Color', rgb(regioncolor{2}), 'LineWidth', 4)
plot([5 15], [50 50], 'Color', rgb(regioncolor{3}), 'LineWidth', 4)
text(20,90, 'Hipp Power or Hipp-BLA Coherence', 'Color', rgb(regioncolor{1}), 'FontName', 'Arial', 'FontSize', 20)
text(20,70, 'Peri Power or Peri-Hipp Coherence', 'Color',  rgb(regioncolor{2}), 'FontName', 'Arial', 'FontSize', 20)
text(20,50, 'BLA Power  or BLA-Peri Coherence', 'Color', rgb(regioncolor{3}), 'FontName', 'Arial', 'FontSize', 20)
set(gca, 'FontWeight', 'bold')
set(gca,'YTick',[])
set(gca,'XTick',[])
axis off
...
recogtest(1).alldata = alldata;%add to recogtest 1 just to return it
